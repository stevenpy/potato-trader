...
RSpec.describe Api::V1::ProfitsController, type: :controller do
  describe "GET #max_potential_profit" do
    let(:date) { "2022-08-22" }
    ...
    context "when data exists for the date" do
      ...
      it "returns the maximum potential profit" do
        get :max_potential_profit, params: {date: date}

        expect(response).to have_http_status(:ok)

        result = JSON.parse(response.body)
        expect(result["max_profit"]).to eq("1000.0")
      end
    end

    context "when no data exists for the date" do
      it "returns a not found error" do
        get :max_potential_profit, params: {date: "2023-01-01"}
    ...
    context "with invalid date format" do
      it "returns a bad request error" do
        get :max_potential_profit, params: {date: "invalid-date"}
    ...
    context "with future date" do
      it "returns a bad request error" do
        ...
        get :max_potential_profit, params: {date: future_date}
